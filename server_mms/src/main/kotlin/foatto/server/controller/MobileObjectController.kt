...
import foatto.server.service.MobileObjectService
...
class MobileObjectController(
    private val mobileObjectService: MobileObjectService,
) {

    @PostMapping(ApiUrlMMS.MOBILE_OBJECT)
    @Transactional
    fun app(
        @RequestBody
        appRequest: AppRequest
    ): AppResponse {
        return mobileObjectService.app(
            sessionId = appRequest.sessionId,
            action = appRequest.action,
        )
    }

    @PostMapping(ApiUrlMMS.MOBILE_OBJECT_FORM_ACTION)
    @Transactional
    fun formAction(
        @RequestBody
        formActionRequest: FormActionRequest
    ): FormActionResponse {
        return mobileObjectService.formAction(
