...
class CompositeObjectListDashboardService(
    ...
    fun composite(
        ...
        val sensorEntities = objectEntity?.let {
            val result = mutableListOf<Pair<DashboardSensorTypeEnum, SensorEntity>>()
            ...
            listOf(
                ...
                sensorRepository.findByObjAndSensorType(objectEntity, sensorType).forEach { sensorEntity ->
                    result += DashboardSensorTypeEnum.ANALOGUE to sensorEntity
                }
            }
            listOf(
                ...
                sensorRepository.findByObjAndSensorType(objectEntity, sensorType).forEach { sensorEntity ->
                    result += DashboardSensorTypeEnum.COUNTER to sensorEntity
                }
            }
            listOf(
                ...
                sensorRepository.findByObjAndSensorType(objectEntity, sensorType).forEach { sensorEntity ->
                    result += DashboardSensorTypeEnum.WORK to sensorEntity
                }
            }
            result
        } ?: emptyList()

        //--- в соотношении 2:1 + столбец со списком объектов
        val blockRows = if (sensorEntities.isEmpty()) {
            2
        } else {
            ceil(sqrt(sensorEntities.size / 2.0)).toInt()
        }
        val blockOnRow = blockRows * 2 + 1

        //--- object list module ---

        val tableAction = AppAction(
            type = ActionType.MODULE_TABLE,
            module = AppModuleMMS.OBJECT_LIST,
            id = action.id,
        )
        val appResponse = objectListService.app(sessionId, tableAction)
        ...
        var row = 0
        var col = 1 // 0-й столбец занимает список объектов
